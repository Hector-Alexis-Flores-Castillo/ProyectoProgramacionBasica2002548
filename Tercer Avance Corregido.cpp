...
{
	int NumArt = 0;
	string NombreArt, Año, Categoria, Genero, Clasificacion, Consola, Descripcion;
...
void alta()
{
	...
	for (int i = 0; i < registros; i++)
	{
		...
		printf("Categoria: ");
		getline(cin, Artículos[i].Categoria);
		printf("A%co de lanzamiento: ", 164);
		getline(cin, Artículos[i].Año);
		printf("Genero: ");
...
void modif()
{
	...
	else
	{
		...
		else
		{
			...
			printf("Categoria: %s\n", Artículos[j].Categoria.c_str());
			printf("A%co de lanzamiento: %s\n", 164, Artículos[j].Año.c_str());
			...
			do
			{
				...
				case 2:
					...
					break;
				case 3:
					printf("Indique nuevas categoria del articulo: ");
					cin.ignore();
					getline(cin, Artículos[j].Categoria);
					break;
				case 4:
					printf("Indique nuevo a%co de lanzamiento del articulo: ", 164);
					cin.ignore();
					getline(cin, Artículos[j].Año);
					break;
				case 5:
	...
	else
	{
		do
		{
			...
			else
			{
				...
				printf("Categoria: %s\n", Artículos[k].Categoria.c_str());
				printf("A%co de lanzamiento: %s\n", 164, Artículos[k].Año.c_str());
...
void lista()
{
	...
	{
		do
		{	//Opcion de aplicar filtro de busqueda.
			printf("Seleccione una accion: \n1. Mostrar lista completa\n2. Aplicar filtro de busqueda\n");
			scanf_s("%d", &seleccion);
		} while (seleccion != 1 && seleccion != 2);

		if (seleccion == 1)
		{
			for (int i = 0; i < registros; i++)		//Impresión de lista completa.
			{
				...
				else
				{
					...
					printf("Categoria: %s\n", Artículos[i].Categoria.c_str());
					printf("A%co de lanzamiento: %s\n", 164, Artículos[i].Año.c_str());
		...
		{
			filtro();	//Impresión de lista con filtros.
		}
	}
	system("pause");
}
...
void filtro()
{
	...
	do
	{	//Seleccion de filtro de busqueda.
		printf("Indique filtro de busqueda:\n1. Categoria\n2. Genero\n3. Consola\n4. Clasificacion\n");
		scanf_s("%d", &seleccion);
	} while (seleccion < 1 || seleccion > 4);
	
	printf("Indique busqueda: ");
	cin.ignore();
	...
	switch (seleccion)
	{
	case 1:		//Filtro por categoria.
		for (int i = 0; i < registros; i++)
		{
			if ((strcmp(busqueda.c_str(), Artículos[i].Categoria.c_str()) == 0) && (Artículos[i].Estado != "Eliminado"))
			{
				printf("\nNumero de articulo: %d\n", Artículos[i].NumArt);
				printf("Nombre del articulo: %s\n", Artículos[i].NombreArt.c_str());
				printf("Descripcion: %s\n", Artículos[i].Descripcion.c_str());
		...
		break;
	case 2:		//Filtro por genero.
		for (int i = 0; i < registros; i++)
		{
			if ((strcmp(busqueda.c_str(), Artículos[i].Genero.c_str()) == 0) && (Artículos[i].Estado != "Eliminado"))
			{
				printf("\nNumero de articulo: %d\n", Artículos[i].NumArt);
				printf("Nombre del articulo: %s\n", Artículos[i].NombreArt.c_str());
				printf("Descripcion: %s\n", Artículos[i].Descripcion.c_str());
		...
		break;
	case 3:		//Filtro por consola.
		for (int i = 0; i < registros; i++)
		{
			if ((strcmp(busqueda.c_str(), Artículos[i].Consola.c_str()) == 0) && (Artículos[i].Estado != "Eliminado"))
			{
				printf("\nNumero de articulo: %d\n", Artículos[i].NumArt);
				printf("Nombre del articulo: %s\n", Artículos[i].NombreArt.c_str());
				printf("Descripcion: %s\n", Artículos[i].Descripcion.c_str());
		...
		break;
	case 4:		//Filtro por clasificacion.
		for (int i = 0; i < registros; i++)
		{
			if ((strcmp(busqueda.c_str(), Artículos[i].Clasificacion.c_str()) == 0) && (Artículos[i].Estado != "Eliminado"))
			{
				printf("\nNumero de articulo: %d\n", Artículos[i].NumArt);
				printf("Nombre del articulo: %s\n", Artículos[i].NombreArt.c_str());
				printf("Descripcion: %s\n", Artículos[i].Descripcion.c_str());
	...
	else
	{
		...
		for (int i = 0; i < registros; i++)
		{
			...
			else
			{
				...
				archivo << Artículos[i].Categoria << "\t";
				archivo << Artículos[i].Año << "\t";
